...
def xml2cps():
    with open('dummy.tmp','w') as d:
        for element in range(1,576):
            call(['/home/proto/Downloads/copasi/bin/CopasiSE','-i','../../atomizer2/XMLExamples/curated/BIOMD%010i.xml' % element],stdout=d)
            print element
...
def correctCPS():
    for element in range(1,576):
        ...
        try:
            cinputFile = open('../../atomizer2/XMLExamples/curated/BIOMD%010i.cps' % element,'r')
        except IOError:
        ...
        document = reader.readSBMLFromFile('../../atomizer2/XMLExamples/curated/BIOMD%010i.xml' % element)
        ...
        molecules,_,_,_, observablesDict, _ = parser.getSpecies({})

        _,_,_,_,removeParams,_ = parser.getAssignmentRules(zparam,param,rawSpecies, observablesDict, {})  
